...
AF_Stepper myStepperY(48,1);            
AF_Stepper myStepperX(24,2);  
...
int StepDelayPenUp = 1; // Разная скорость будет для положения ручки внизу и вверху для повыщения точности в моменты рисования
int StepDelayPenDown = 5 ;// Чем больше тем более качественно будет
float StepInc = StepDelayPenUp;
int StepDelay = 2;//было 0
int LineDelay =0;
int penDelay = 50;
...
//float StepsPerMillimeterX = 2.94;
//float StepsPerMillimeterY = 1.89;
float StepsPerMillimeterX = 30;//34  Сейчас стоят имперически верные значения
float StepsPerMillimeterY = 48;//53
      ...
      case 300:
        {
          ...
          if (Spos == 50) { 
           ...
           if (Spos == 255) { 
             penServo.detach();// turn off servo
          }
